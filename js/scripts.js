...
function screenMenu(){
		var subWidth = $('.sub_menu_wrap').width();
		var timeout;

		function getWidth(block) {
			var width = 0;
			block.find('>*').each(function(i, item){
				width += $(item).width();
			});
			return width;
		}
		function getWidth2(block) {
			var width = 0;
			block.find('>.time, .sub_menu').each(function(i, item){
				width += $(item).width();
			});
			return width;
		}

		$('.dropdown-all-menu').hover(function(e){
			var $sub = $(this).parent().find('.sub_menu_wrap'),
				$preview = $sub.find('.preview');
			$preview.fadeOut(0);
			$sub.find('.time').remove();
			$sub.width(getWidth($sub));
			$sub.find('>*').height('auto')
			setEqualHeight($sub.find('>*'));
			$sub.children('.preview').fadeIn(0);
		}, function(){
			
		});

		$('.sub_menu>li').not('.has-two').hover(function(e){
			var $li = $(this),
				$sub = $li.parents('.sub_menu_wrap'),
				$preview = $sub.find('.preview');
			...
			timeout = setTimeout(function(){
				if($li.has('.preview')) {
					...
					$sub.find('.time').remove();
					var liTime = $li.children('.preview').clone();
					$sub.append(liTime.addClass('time'));
					$sub.width(getWidth2($sub));
					$preview.fadeOut(0);
					$sub.find('>*').height('auto')
					setEqualHeight($sub.find('>*'));
					$sub.find('.preview.time').fadeIn(0);					
				}
			}, 100);
		}, function(){
		...
		$('.sub_menu>li.has-two').hover(function(e){
			var $li = $(this),
				$sub = $li.parents('.sub_menu_wrap'),
				$preview = $sub.find('>.preview'),
				$twoMenu = $sub.find('>.two-level');
			...
			timeout = setTimeout(function(){
				if($li.has('.two-level')) {
					...
					$sub.find('.time').remove();
					var liTime = $li.children('>.preview').clone(),
						ulTime = $li.children('>.two-level').clone();
					$sub.append(ulTime.addClass('time'));	
					$sub.append(liTime.addClass('time'));
					$sub.width(getWidth2($sub));
					$sub.find('.preview, .two-level').fadeOut(0);
					if($li.has('.two-level')) {
						$sub.width(subWidth);
						$sub.find('.time').remove();
						var liTime = $li.children('.preview').clone(),
		...
		function twoLevel() {
			$('.two-level li').hover(function(e){
				...
				timeout = setTimeout(function(){
					$sub.width(subWidth);
					$sub.find('.preview.time').remove();
					var liTime = $li.children('>.preview').clone();	
					$sub.append(liTime.addClass('time'));
					$sub.width(getWidth2($sub));
					$sub.find('.preview').fadeOut(0);
					if($li.has('.preview')) {
						$sub.width(subWidth);
						$sub.find('.preview.time').remove();
						var liTime = $li.children('.preview').clone();
...
rMenu.openCloseSubMenu = function(e){
	...
	if(self.next('.sub_menu_wrap, .two-level').length){
		self.parent()
